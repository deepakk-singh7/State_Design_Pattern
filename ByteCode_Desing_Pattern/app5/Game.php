...
class Game {
    ...
    private array $log = [];
    private array $currentTurnLog = []; // New log for current turn only
    ...
    public function getName(int $wizardId): string { 
        return $this->wizards[$wizardId]['Name']; 
    }
    ...
    public function setWisdom(int $wizardId, int $amount): void { 
        $this->wizards[$wizardId]['Wisdom'] = max(0, $amount); 
    }
    
    public function setAgility(int $wizardId, int $amount): void { 
        $this->wizards[$wizardId]['Agility'] = max(0, $amount); 
    }

    public function addToLog(string $message): void { 
        $this->log[] = $message;
        $this->currentTurnLog[] = $message; // Also add to current turn log
    }
    ...
    public function getWinner(): ?string {
        if ($this->wizards[0]['Health'] <= 0 && $this->wizards[1]['Health'] <= 0) 
            return "It's a draw!";
    ...
    public function getState(): array {
        $state = [
            ...
            'log' => $this->currentTurnLog, // Send only current turn log
            'isOver' => $this->isOver(),
            'winner' => $this->getWinner()
        ];
        $this->currentTurnLog = []; // Clear current turn log after getting state
        return $state;
    }
